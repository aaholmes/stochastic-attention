...
- **Skip-K / clustering (closed, negative)** — estimating a cluster's mass from its *moments* is dead (mass sits on one token per cluster); a *magnitude* estimate ranks the hot clusters well, but the end-to-end test killed it: deterministically selecting clusters needs ~60% of keys to preserve quality and collapses below that, because attention's **heavy tail** can't be dropped. Plain sampling (which *samples* the tail instead of reading/dropping it) wins by ~10×.

**Result — we tested the skip-K hypothesis end-to-end, and it failed (cleanly).** We wired magnitude-ranked cluster selection (`skip_k`) into the model and measured perplexity vs key-read fraction. Plain sampling won by ~10×: `santa_sys` reaches dense at ~9% reads, while deterministic cluster selection needs ~60% reads to match and *collapses* (+92% perplexity) by 17%. The reason is structural and worth knowing: the magnitude estimate *ranks* the hot clusters fine, but attention's **heavy tail** (thousands of tiny weights) carries enough *aggregate* mass that *dropping* the un-selected clusters wrecks the output — and you can't *skip* those keys without sampling (i.e. reading) them. That is exactly *why* unbiased sampling wins: it captures the tail's contribution by sampling it cheaply rather than reading or discarding it. **So the clustering / skip-K line is closed (negative); the champion stands: plain unbiased systematic sampling, dense quality at ~3.5–9% reads.** The lasting value of the clustering arc is the *negative knowledge* (why the tail forbids skipping key-reads) and the reusable diagnostics/derivations. Remaining open directions (untested) are the other ideas: reuse/resident caching, cross-head sharing, the large value-memory architecture, and a real gather-kernel microbenchmark.
